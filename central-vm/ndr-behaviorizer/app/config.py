...
class Settings:
    ...
    ml_retrain_interval_seconds: int = 21600
    ml_force_retrain: bool = False
    finding_suppressed_statuses: str = "false_positive,benign_expected"
    training_excluded_feedback_statuses: str = "true_positive"
...
def load_settings(config_path: str | None = None) -> Settings:
    ...
    return Settings(
        ...
        ml_retrain_interval_seconds=_int(os.getenv("ML_RETRAIN_INTERVAL_SECONDS"), int(_get(cfg, "ml_retrain_interval_seconds", Settings.ml_retrain_interval_seconds))),
        ml_force_retrain=_bool(os.getenv("ML_FORCE_RETRAIN", str(_get(cfg, "ml_force_retrain", Settings.ml_force_retrain))), Settings.ml_force_retrain),
        finding_suppressed_statuses=os.getenv("FINDING_SUPPRESSED_STATUSES", _get(cfg, "finding_suppressed_statuses", Settings.finding_suppressed_statuses)),
        training_excluded_feedback_statuses=os.getenv("TRAINING_EXCLUDED_FEEDBACK_STATUSES", _get(cfg, "training_excluded_feedback_statuses", Settings.training_excluded_feedback_statuses)),
